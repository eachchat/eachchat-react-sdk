...
import { getNoLabelValues, buildGraphQLQuery, labelValueRenderer } from '../qxp/utils';
import { LabelValue, Employee, Option } from '../qxp/interface';
import { searchUser, getUserDetail } from './request';
...
const UserPicker = ({
    ...
    // const currentUser = {
    //     label: "赵冰玉",
    //     value: "039A91BFC1DA407CAEBB24D9C5FB8EE4",
    //     email: "zhaobingyu@yunify.com",
    // };
    ...
    useEffect(() => {
        ...
        if (optionalRange === 'currentUser' || defaultRange === 'currentUser') {
            currentUser && onChange?.([omit(currentUser, 'email')]);
    ...
    if (optionalRange === 'currentUser') {
        currentUser && (componentsProps.options = [currentUser]);
    }
...
const AllUserPicker = ({ appID, value, ...otherProps }: AllUserPickerProps): JSX.Element | null => {
    ...
    useEffect(() => {
        ...
        //     page,
        // })
        // cli?.getQXPUserList(params.page, PAGE_SIZE)
        //     .then((res) => {
        //         if (res) {
