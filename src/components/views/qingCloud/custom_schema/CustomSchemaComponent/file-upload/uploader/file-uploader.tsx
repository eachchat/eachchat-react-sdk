...
function FileUploader({
    ...
    function beforeUpload(preUploadFile: File, files: File[], storeFiles: QXPUploadFileBaseProps[]): boolean {
        ...
        if (accept && !isAcceptedFileType(preUploadFile, accept)) {
            message.error(`文件 '${preUploadFile.name}' 的格式不正确`);
            return false;
        }

        if (storeFiles.find((file) => file.name === preUploadFile.name)) {
            message.error(`已存在名为'${preUploadFile.name}' 的文件。`);
            return false;
        }
        if (!multiple && (files.length !== 1 || storeFiles.length === 1)) {
            message.error('仅允许上传一个附件');
            return false;
        }
        ...
        if (multiple) {
            ...
            if (maxSize && uploadedTotalSize > maxSize) {
                message.error(`文件总大小不能超过${maxFileSize}MB`);
                return false;
            }
        }

        if (maxSize && preUploadFile.size > maxSize) {
            message.error(`单个文件大小不能超过${maxFileSize}MB`);
            return false;
        }
        return true;
    }
...
export default React.forwardRef<HTMLDivElement, FileUploaderProps>(FileUploader);
