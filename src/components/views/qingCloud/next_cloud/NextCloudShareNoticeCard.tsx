            ...
            >
{
                    isShareLink ?
                        ...
                        <Card title={<Title />}  size="small">
                             <div className="content pointer hover-active" onClick={() => this.onBodyLinkClick(SdkConfig.get("setting_defaults")?.QingCloud?.next_cloud_url || SdkConfig.get("setting_defaults").next_cloud_url)}>
                                {isFolder ? <FolderOutlined /> : <FileOutlined />}
                                <span className="name">  {fileName} </span>
                            </div>
