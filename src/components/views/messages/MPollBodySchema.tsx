...
export default class MPollBodySchema extends React.Component<IBodyProps, IState> {
    ...
    private selectOption(answerId: string) {
        ...
        // 添加处理意见字段
        const { schema_type } = this.props.mxEvent?.event?.content || {};
        response.content.dealReason = this.state.dealReason;
        response.content.schema_type = schema_type;
    ...
    private onOptionClick = (): void => {
        const message = _t("The poll has ended. No votes were cast.");
        const endEvent = PollEndEvent.from(this.props.mxEvent.getId(), message).serialize();
        this.context.sendEvent(
            this.props.mxEvent.getRoomId(), endEvent.type, endEvent.content,
        ).catch((e: any) => {
            console.error("Failed to submit poll response event:", e);
    ...
    // TODO:
    private getDealReason(): string {
        let dealReason='';
        try {
            const voteRelations =this.fetchVoteRelations();
            const res = voteRelations.getRelations().filter(isPollResponse);
            dealReason = res[res.length-1]?.event?.content?.dealReason;
            return dealReason;
        } catch (error) {
            return dealReason;
        }
    ...
    private handleDealReason = (e) => {
        this.setState({ dealReason: e.target.value });
    };
    ...
    render() {
        ...
        // TODO:
        const { schema_view_form, initial_value, answers_textarea } = this.props.mxEvent?.event?.content || {};
        return <div className="mx_MPollBody mx_MPollBody_schema">
            <h2>{ poll.question.text }{ editedSpan }</h2>
            <SchemaForm
                schema={schema_view_form}
                initialValues={initial_value}
                edit={false} />
            {
                !!answers_textarea &&
                <div className='mx_MPollBody_schema_deal_reason_wrap'>
                    <span>{ answers_textarea }：</span>
                    {
                        ended?
                            <div className='deal_reason_text'>{ this.state.dealReason }</div>:
                            <TextArea
                                rows={4}
                                disabled={ended}
                                value={this.state.dealReason}
                                onChange={this.handleDealReason} />
                    }

                </div>
            }

            <div className="mx_MPollBody_allOptions mx_MPollBody_allOptions_schema">
                {
...
interface ILivePollOptionProps {
    ...
    onOptionClick?: (e: React.FormEvent<HTMLInputElement>) => void;
}
